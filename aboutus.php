
<!doctype html>
<html lang="en">

 <?php include('include/headerscript.php'); ?>
   <body>
   <?php include('include/header.php'); ?>

  
    <!-- Page Breadcrumbs Start -->
    <section class="breadcrumbs-page-wrap">        
        <div class="bg-navy-blue bg-fixed pos-rel breadcrumbs-page">
            
            <div class="container">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="index.php">Home</a></li>
                        <li class="breadcrumb-item active" aria-current="page">About Us</li>
                    </ol>
                </nav>
                <h1>About Us</h1>
            </div>
        </div>
    </section>
    <!-- Page Breadcrumbs End -->

    <!-- Main Body Content Start -->
    <main id="body-content">


    <section class="section-content-block">

            <div class="container">

                <div class="row">

                    <div class="col-sm-6">
                        <img src="assets/images/aboutus.jpg" alt="girl playing with dog">
                    </div>
                    
                    <div class="col-md-6 col-sm-12 text-left">
                        
                        <div class="about-us-intro about-us-overlay theme-custom-box-shadow">

                        <p class="our-experience">
                                Our Trusted Name To
                                <br>
                                Pet Lovers Over <span> 10 Years </span> For
                                <br>
                                adoption / veterinary  services.
                            </p>

                            Petpaw was founded July 13, 2009.We rescue about 200 dogs each year, and the majority of them arrive at Mattie's Place with severe medical requirements and would otherwise be euthanized in their current circumstances.
                            We don't make any money from Pet Paw Place, but if you'd like to donate to help us aid more pets in need, we'd appreciate it. Donations will be used to accept more creatures facing euthanasia into our adoption programme and to help offset the cost of medical treatment for the majority of our dogs and cats who are medical emergencies.  
                        </div>  
                    </div> <!-- end .col-sm-12  --> 

                </div>

                <div class="row margin-top-32">

                     <div class="col-lg-3 col-6">

                        <div class="counter-block-2 counter-border">

                            <i class="icon pethund_pet_belt"></i>
                            <div class="count-info">
                                <span class="counter">35</span> 
                                <span class="counter-postfix">+</span>   
                                <p class="text-capitalize">Dedicated Doctors</p>
                            </div>

                        </div>

                    </div> 

                    <div class="col-lg-3 col-6">

                        <div class="counter-block-2 counter-border">

                            <i class="icon pethund_pet_professional"></i>

                            <div class="count-info">
                                <span class="counter">7000</span>
                                <p class="text-capitalize">Happy Customers</p>
                            </div>

                        </div>

                    </div> 

                    <div class="col-lg-3 col-6">

                        <div class="counter-block-2 counter-border">

                            <i class="icon pethund_pet_awards"></i>
                            <div class="count-info">
                                <span class="counter">99</span> 
                                <span class="counter-postfix">%</span>   
                                <p class="text-capitalize">Success Rate</p>
                            </div>

                        </div>

                    </div> 

                    <div class="col-lg-3 col-6">

                        <div class="counter-block-2 counter-border">

                            <i class="icon pethund_pet_salon"></i>
                            <div class="count-info">
                                <span class="counter">1275</span>    
                                <p class="text-capitalize">National Awards</p>
                            </div>

                        </div>

                    </div> 

                </div>     

            </div> 

        </section>
         <!-- Vision Mission Start -->
         <section class="wide-tb-100 pt-0">
            <div class="container">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="vision-mission">
                            <img src="assets/images/vision2.jpg" alt="vision image" class="img-vision">
                            <h2 class="txt-blue fw-7">Our Vision</h2>
                            <p>Positive Attitude.Greeting each day with our joy on our face,love in our heart and infectious smile,looking forward to positive side of various situation and challenges.positive work surronding by empowering eachother .Dedication towards cilents and their pet is main focus for our company. </p>
                        </div>

                        <div class="trophy-awards">
                            <i class="pethund_pet_awards"></i>
                            <h4>World Best Class Treatment & Award Wining Services </h4>
                        </div>
                    </div>
                    <div class="col-lg-6 flex-lg-column-reverse d-lg-flex">                        
                        <div class="vision-mission">
                            <img src="assets/images/mission.jpg" alt="mission image" class="img-vision">
                            <h2 class="txt-orange fw-7">Our Mission</h2>
                            <p>Team work mentality. Working as a team to create positive work surrounding by empowering each other. Dedication towards clients and their pets is main focus of our company.</p>
                        </div>
                        <div class="best-petcare">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -32 512.00001 512"><path d="m342.382812 239.351562c-23.039062-35.941406-62.277343-57.402343-104.964843-57.402343-42.683594 0-81.925781 21.460937-104.960938 57.402343l-55.515625 86.605469c-9.210937 14.371094-13.460937 30.96875-12.292968 47.996094 1.167968 17.03125 7.648437 32.890625 18.738281 45.871094 11.097656 12.976562 25.761719 21.84375 42.40625 25.648437 16.644531 3.800782 33.707031 2.179688 49.339843-4.691406l1.019532-.453125c39.339844-16.957031 84.304687-16.804687 123.546875.453125 10.121093 4.449219 20.84375 6.699219 31.664062 6.699219 5.882813 0 11.800781-.667969 17.664063-2.003907 16.644531-3.800781 31.308594-12.667968 42.410156-25.644531 11.09375-12.976562 17.578125-28.839843 18.75-45.871093 1.171875-17.035157-3.078125-33.632813-12.289062-48.007813zm26.246094 160.972657c-14.121094 16.507812-36.964844 21.726562-56.847656 12.984375-23.632812-10.394532-49-15.589844-74.375-15.589844-25.351562 0-50.714844 5.191406-74.332031 15.574219l-.671875.296875c-19.730469 8.34375-42.238282 3.058594-56.203125-13.265625-14.105469-16.511719-15.710938-39.886719-3.992188-58.171875l55.519531-86.605469c17.492188-27.289063 47.28125-43.582031 79.691407-43.582031 32.410156 0 62.203125 16.292968 79.699219 43.582031l55.511718 86.601563c11.722656 18.292968 10.113282 41.671874-4 58.175781zm0 0"></path><path d="m91.894531 239.238281c16.515625-6.34375 29.0625-19.652343 35.332031-37.476562 5.960938-16.960938 5.472657-36.109375-1.382812-53.921875-6.859375-17.800782-19.335938-32.332032-35.128906-40.921875-16.597656-9.019531-34.828125-10.488281-51.316406-4.132813-33.171876 12.753906-48.394532 53.746094-33.929688 91.398438 11.554688 29.96875 38.503906 48.886718 65.75 48.886718 6.957031 0 13.933594-1.234374 20.675781-3.832031zm-58.417969-55.835937c-8.523437-22.1875-1.035156-45.789063 16.703126-52.609375 3.203124-1.234375 6.589843-1.847657 10.046874-1.847657 5.335938 0 10.847657 1.457032 16.152344 4.34375 9.539063 5.183594 17.160156 14.183594 21.457032 25.335938 4.292968 11.160156 4.675781 22.941406 1.074218 33.179688-3.300781 9.382812-9.617187 16.28125-17.78125 19.417968l-.015625.007813c-17.714843 6.828125-39.085937-5.660157-47.636719-27.828125zm0 0"></path><path d="m199.613281 171.386719c41.46875 0 75.207031-38.4375 75.207031-85.683594 0-47.257813-33.738281-85.703125-75.207031-85.703125-41.464843 0-75.199219 38.445312-75.199219 85.703125 0 47.246094 33.734376 85.683594 75.199219 85.683594zm0-141.375c24.917969 0 45.195313 24.984375 45.195313 55.691406 0 30.695313-20.277344 55.671875-45.195313 55.671875s-45.1875-24.976562-45.1875-55.671875c0-30.707031 20.269531-55.691406 45.1875-55.691406zm0 0"></path><path d="m329.496094 192.4375h.003906c6.378906 2.117188 12.886719 3.128906 19.367188 3.128906 30.242187 0 59.714843-22.011718 70.960937-55.839844 6.476563-19.472656 6.050781-40.0625-1.199219-57.972656-7.585937-18.746094-21.644531-32.355468-39.589844-38.324218-17.945312-5.960938-37.363281-3.476563-54.664062 7-16.527344 10.011718-29.191406 26.246093-35.65625 45.71875-13.652344 41.078124 4.640625 84.273437 40.777344 96.289062zm-12.296875-86.824219c4.222656-12.714843 12.292969-23.191406 22.726562-29.511719 9.652344-5.847656 20.183594-7.335937 29.648438-4.191406 9.460937 3.148438 17 10.640625 21.234375 21.101563 4.574218 11.304687 4.769531 24.53125.539062 37.246093-8.433594 25.375-31.933594 40.492188-52.382812 33.699219-20.433594-6.796875-30.199219-32.96875-21.765625-58.34375zm0 0"></path><path d="m487.875 182.4375-.011719-.011719c-28.597656-21.125-71.367187-11.96875-95.347656 20.421875-23.957031 32.40625-20.210937 75.972656 8.34375 97.113282 10.414063 7.714843 22.71875 11.402343 35.3125 11.402343 21.949219 0 44.785156-11.203125 60.046875-31.804687 23.957031-32.40625 20.214844-75.972656-8.34375-97.121094zm-15.777344 79.265625c-14.160156 19.113281-38.101562 25.453125-53.378906 14.136719-15.265625-11.300782-16.195312-36.042969-2.074219-55.144532 9.386719-12.679687 23.097657-19.734374 35.734375-19.734374 6.390625 0 12.507813 1.804687 17.648438 5.605468 15.253906 11.3125 16.179687 36.046875 2.070312 55.136719zm0 0"></path></svg>
                            <h4>Best Pet Care Services Since 2009</h4>
                        </div>
                    </div>
                </div>
            </div>

        </section>

        <section class="wide-tb-150 bg-scroll pos-rel callout_bg2">
            <div class="bg-overlay black opacity-30"></div>
            <div class="container">
                <div class="text-center">
                   
                    <div class="col-sm-12 wow fadeInDown" data-wow-duration="0" data-wow-delay="0s" style="visibility: visible; animation-delay: 0s; animation-name: fadeInDown;">
                    <div class="row">
        <div class="col-md-4 col-sm-6">
            <div class="serviceBox">
                <div class="service-icon">
                    <span>1</span>
                </div>
                <h3 class="title">Bring Pets</h3>
                <p class="description">When a pet needs a treatment bring to us we will look after your furry friend and will help you with the best treatment from our side.</p>
            </div>
        </div>
        <div class="col-md-4 col-sm-6">
            <div class="serviceBox green">
                <div class="service-icon">
                <span>2</span>
                </div>
                <h3 class="title">Diagnosis Problems</h3>
                <p class="description">After, bringing your pet with us we will find out the problem what your pet has and look after it.</p>
            </div>
        </div>
        <div class="col-md-4 col-sm-6">
            <div class="serviceBox red">
                <div class="service-icon">
                <span>3</span>
                </div>
                <h3 class="title">Provide Treatments</h3>
                <p class="description">Our Vets will try there best to get the best treatment your pet.And they will be cure soon with the happy face.</p>
            </div>
        </div>
    </div>
                      
                    </div>

                    <a href="appoitment.php" class="btn-theme bg-green btn-shadow" style="margin-top:30px;">Book An Appointment</a>
                   
                </div>
            </div>
        </section>

       

     


    </main>

    <?php include('include/footer.php'); ?>
         <?php include('include/footerscript.php'); ?>
</body>
</html>